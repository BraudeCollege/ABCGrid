...
/**
 * Append the support to File permissions.
 * */
...
TarArchive extends Object
    ...
    /**
     * Add by Suny to support file permission
     *
     * */
    private void chmod(int mode,File f)
    {
        String osname = System.getProperty( "os.name" );
        String Win32Prefix = "windows";
        //not change mode in Windows
        if ( osname.toLowerCase().startsWith( Win32Prefix ))
            return;
        try
        {
            //oct(33261)->dec(100755)->The last three bit->755
            String ss = String.format("%o",mode);

            String smode = ss.substring(ss.length()-3,ss.length());
            ProcessBuilder pb = new ProcessBuilder("chmod",smode,f.getName());
            pb = pb.directory(f.getParentFile());
        ...
        {
            System.out.println("Failed to change file mode, " +
                    "the application may not be invoked...");
        }
    }
